...
- **V5 定制重点（规格已锁定，分支 `feat/v5`）**：将现有 Web GUI **包装为桌面壳**（Electron），与 **浏览器交付** 并存、功能对等；首版 macOS + Windows。PRD 见 `docs/prd/desktop-v5.md` 与 Issue [#111](https://github.com/NanGePlus/my-deepseek-harness/issues/111)。ADR 见 `docs/adr/0009-desktop-shell-electron-delivery.md`、`docs/adr/0010-desktop-browser-electron-cdp.md`；领域词汇见 `CONTEXT.md`（**桌面壳**、**面板内 WebView** 等）。实现切片：GitHub [#111](https://github.com/NanGePlus/my-deepseek-harness/issues/111)（#113–#122；#112 重复已关闭）。**`desktop` profile**（`dsh --profile desktop --dump-config`）已落地：Host 能力与 web 对等（apiproxy、Playwright、终端/Git/浏览器 client roster），**不含** `dsh-host-webserver` 与 loopback HTTP 传输行。**Issue [#116](https://github.com/NanGePlus/my-deepseek-harness/issues/116)**：`IpcApiClient` 统一 IPC carrier（`callUnary`/`respond` + mux/host/watchPath/`terminalStream`/`browserWatchScreencast` 下行；SSE 不走 unary `response.text()`）；integrated 模式 preload 暴露 `window.dsh` IPC 桥；attach 模式仍用 `WebApiClient`。**Issue [#117](https://github.com/NanGePlus/my-deepseek-harness/issues/117)**：**标准壳**：`requestSingleInstanceLock` 二次启动聚焦主窗口；关窗/Quit **关闭即退出**（attach 模式仅退出 GUI）；dirty 编辑器 **退出守卫** 复用 Session 切换守卫对话框；`desktop.windowBounds.v1` 窗口几何持久化；About / Settings / Quit 应用菜单；Dock/Taskbar 图标。**Issue [#119](https://github.com/NanGePlus/my-deepseek-harness/issues/119)**：桌面 **浏览器** 段 occupant 为 `#browser-occupant` 面板内 WebView 占位，经 preload `reportBrowserOccupantBounds` 驱动 Main BrowserView bounds；无「显示窗口」卡片；切走段 `visible=false` detach 且不关 Host Tab（已合并 PR [#130](https://github.com/NanGePlus/my-deepseek-harness/pull/130)）。**Issue [#120](https://github.com/NanGePlus/my-deepseek-harness/issues/120)**：`electron-builder` 打包（macOS **dmg**、Windows **NSIS**）；`extraResources` 捆绑 `apps/web` dist、Playwright Chromium 与 Host runtime；Main 在 packaged 模式设置 `PLAYWRIGHT_BROWSERS_PATH`；CI `desktop-packaging.yml` 产出 unsigned artifact 并跑 headless smoke。
## 已实现定制功能（相对上游 master）
...
| `host.browserList` / `host.browserCreateTab` / `host.browserCloseTab` / `host.browserSelectTab` / `host.browserShowWindow` | Workspace 级 Playwright Tab 池列举 / 新建 / 关闭 / 选中；`showWindow` 与选中/导航会 `bringToFront` 有头窗口；`createTab` 不使用 30s 一元超时 |
| `host.browserNavigate` / `host.browserGoBack` / `host.browserGoForward` / `host.browserReload` | 导航与历史；响应含 `canGoBack` / `canGoForward`；`reload` 可选 `hard`；页面加载类 RPC 不使用 30s 一元超时 |
...
| `@deepseek-ai/dsh-client-ui-browser` | `packages/bundle/web-app/cordis.patch.yml` 行 `ui-browser` | 工具箱 **浏览器** 段：Workspace Tab store；导航遥控有头 Chromium 窗口 + 多 Tab + 空态/不可用；桌面会话打开网页后选中对应 Tab | 2026-08 |
...
| 文件/目录 | 改了什么 | 日期 |
|-----------|----------|------|
| `packages/bundle/desktop-app/` | **新包**（#114）：`desktop` profile 组合包；`cordis.patch.yml` 挂载与 web 对等的 Host + client roster，**不含** webserver / modules / client-hmr；**2026-08-31** 挂载 `connection`（node 半面无 webServer 时 idle，client 半面进 SPA boot graph）；目录选取改 `-native`（`-auto` 依赖 webServer） | 2026-08-31 |
...
| `packages/host/apiproxy/src/browser-registry.ts` | **#118**：`browserDelivery` web/desktop 分叉；desktop 走 `connectOverCDP`，跳过 `bringToFront`；**2026-08-31** 桌面 `revealTab` 调用 `surface.selectTab`；**2026-08-31** 桌面 `closeTab` 先 `surface.closeTab`；**2026-08-31** 桌面 `createTab` 在 `ensureTab` 后不再二次 Playwright `goto`；**2026-09-01** 拒绝 `chrome-error://` net-error URL 写回 tab 元数据；**2026-09-02** 桌面 navigate 改走 `ensureTab`；net-error 时 Host 回退 `about:blank` 而非抛错 | 2026-09-02 |
| `packages/host/apiproxy/src/fetch/client.ts` | **2026-08-31** `host.gitCommit({push})` / `gitPush` 走 `caller-signal-only`；**2026-08-31** 浏览器页面加载类 RPC 同样免除 30s 一元超时 | 2026-08-31 |
...
| `apps/web/vite.desktop-dev.config.ts` | **新文件**（#115）：dev:desktop 注入 boot graph；**2026-08-31** `/plugins/*` 静态 bundle + 每次请求重读 boot graph 的 `transformIndexHtml`（不再短路 index.html） | 2026-08-31 |
...
| `apps/cli/src/profile-boot.ts` | **2026-08-31**（#120）`INSTALL_ANCHOR` / `SHIPPED_PRESET_ROOT` 兼容 `src/` 与 `lib/types/` 布局 | 2026-08-31 |
| `package.json` | **2026-08-31**（#120）`prepare:desktop-packaging` / `pack:desktop` / `smoke:desktop-packaging` | 2026-08-31 |
| `scripts/prepare-desktop-packaging.ts` | **新文件**（#120）：staging web-dist + Playwright Chromium + Host deploy | 2026-08-31 |
| `scripts/run-electron-builder.mjs` | **新文件**（#120）：绕过 pnpm filter hooks 调用 electron-builder CLI | 2026-08-31 |
| `scripts/smoke-desktop-packaging.ts` | **新文件**（#120）：CI headless artifact smoke | 2026-08-31 |
| `.github/workflows/desktop-packaging.yml` | **新文件**（#120）：macOS dmg + Windows NSIS unsigned CI | 2026-08-31 |
| `pnpm-workspace.yaml` | **2026-08-31**（#115）`allowBuilds.electron`；**#120** `electron-winstaller` | 2026-08-31 |
...
| `packages/client/connection/` | **2026-08-26** `host.watchPath` 浏览器下行改 WebSocket，避免 HTTP/1.1 六连接占满后文件树 listing 排队超时；**2026-08-30** connection schema 与 fetch client 扩展 `host.browser*`；**2026-08-31** `host.browserShowWindow`；**2026-08-31** node 半面 `inject` 不再强制 `webServer`（桌面 Host 可扫描 `dsh.client`，HTTP 路由等 webServer 才挂）；**2026-08-31** desktop IPC 下行增加 `terminalStream` / `browserWatchScreencast`；**2026-09-02** desktop IPC 覆盖 `connection.rpc`（消息反馈等 Typert Remote） | 2026-09-02 |
...
| `packages/client/ui-browser/` | **新包**（#96–#98）：`conversation.details.browser` occupant；Workspace 级 Tab store；首次进入 `about:blank` + 地址栏 focus；Tab 栏 + 右键批量关闭 + 导航栏；**2026-08-31** 人类主表面改为有头 Chromium：内容区「显示窗口」调用 `browserShowWindow`，不再订阅 screencast 或转发指针/键盘；不可用卡片 + 导航错误 + 外部站点 info；未绑定空态；`packages/bundle/web-app` 注册；**2026-08-31** Host 池空时 `+` 先按 store URL 重建再开新 Tab，`browser-tab-not-found` 关 Tab 仍从 store 去掉；**#119** 桌面交付检测 preload `reportBrowserOccupantBounds`：occupant 改 `#browser-occupant` + bounds 上报，移除「显示窗口」主路径；**#122** 桌面 loading「正在准备浏览器…」+ States 矩阵 seam 测试；浏览器交付路径不变；**2026-08-31** 桌面会话链接进内置浏览器；**2026-08-31** 打开后 `setSelectedTab` + `browserSelectTab` 对准新建或复用 Tab；**2026-08-31** 导航 / 会话打开在 `browser-tab-not-found` 时恢复活 Tab，Host 未就绪时排队；**2026-09-01** 移除导航栏「…」溢出菜单（Hard Reload / Copy URL / Zoom）；**2026-09-01** 桌面端 Tab 右键 / tooltip 向上展开；**2026-09-01** store 忽略 `chrome-error://` tab 元数据；**2026-09-02** 桌面 unreachable URL（如 `ERR_CONNECTION_REFUSED`）Host 回退 `about:blank`、不抛 RPC 失败，Client 不显示导航红条/遮罩，「+」新建 Tab 不被阻塞 | 2026-09-02 |
| `packages/client/ui-conversation/` | 工具箱 segmented Tab 壳层；**2026-08-30** #95 扩为五段 **资源管理器 | Git面板 | 终端 | 浏览器 | 工具详情**；声明 `conversation.details.browser`；向浏览器段传入 `visible`（切走只隐藏、不卸载）；**2026-08-31** `revealBrowserSegment` 供桌面会话链接切到浏览器段 | 2026-08 |
...
| `docs/prd/desktop-v5.md` | 桌面壳 V5 PRD（`feat/v5`）；**2026-08-31** 会话 http(s) 链接进面板内 WebView；**2026-08-31** 打开后选中对应 Tab；页面加载类 browser RPC 不走 30s 一元超时 | 2026-08-31 |
...
| `test/desktop-v5-validation` | V5 桌面壳功能验证 | 进行中；基线 `origin/custom/main`（`a2719c3c4f`） |
...
| 2026-09-01 | 分支 `test/desktop-v5-validation`：工具箱浏览器下拉 / tooltip 被网页挡住或下压网页 | Electron `BrowserView` 叠在 Renderer 之上，inset occupant 会把网页往下顶。桌面端改为菜单与 tooltip 向上展开，BrowserView 保持满格 occupant |
| 2026-08-31 | 分支 `test/desktop-v5-validation`：地址栏 `browser tab not found: <uuid>` | Client store 的 tabId 在 Host 重启或丢 Tab 后过期；导航不像刷新那样恢复。改为 `tab-not-found` 时 list/重建后重试；Host 未就绪的提交先排队 |
| 2026-08-31 | 分支 `test/desktop-v5-validation`：浏览器 `signal timed out` 且会话打开不切 Tab | 外部站导航踩 30s 一元超时（Chromium 文案）；RPC 中止后 Client 拿不到新 tabId。页面加载类 browser RPC 改为 `caller-signal-only`；桌面 create 不再二次 `goto`；打开后 `browserSelectTab` |
| 2026-08-31 | 分支 `test/desktop-v5-validation`：会话点链接打开外部窗口 | Markdown / WebBlock `target=_blank` 在 Electron 新开窗口。改为 Main 拦截 http(s) popup，在工具箱内置浏览器打开；导航栏「在外部浏览器打开」仍走系统浏览器 |
| 2026-08-31 | 分支 `test/desktop-v5-validation`：工具箱浏览器输入 URL 后空白 | 页面已在 BrowserView 加载（CDP 可见完整 DOM），但 `revealTab` 在 desktop 直接 return，从未 `selectTab`/`setBounds`，视口 0×0。改为 desktop `revealTab` 调用 `DesktopBrowserSurface.selectTab` |
| 2026-08-31 | 分支 `test/desktop-v5-validation`：全 App 无法复制粘贴 | `Menu.setApplicationMenu` 只有 About/Settings/Quit，替换掉 Chromium 默认 Edit；补 `role: 'editMenu'` |
| 2026-08-31 | 分支 `test/desktop-v5-validation`：终端一直「连接中」、浏览器 `waitForEvent('page')` 超时 | `IpcApiClient` 对 `terminalStream` 仍走 unary fetch，Main `response.text()` 等不到 SSE 结束；BrowserView `about:blank` 不 loadURL，CDP `getURL()` 为空，已有 page 不再发 `page` 事件。改为 IPC 流 + URL 规范化匹配已有 CDP page |
| 2026-08-31 | 分支 `test/desktop-v5-validation`：`dev:desktop` 空白页 | Vite 短路 index.html 丢掉 React preamble；sandbox preload 是 ESM；desktop profile 未挂 `connection` 导致 SPA 等 `ctx.connection`。改为 `transformIndexHtml` + `/plugins/*`；preload CJS；desktop 挂 connection（node idle） |
| 2026-08-31 | 从最新 `origin/custom/main` 创建分支 `test/desktop-v5-validation` | V5 桌面壳功能验证；基线 `a2719c3c4f`（Issue #122 / PR #133 已合并） |
