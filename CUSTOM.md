...
| `host.browserNavigate` / `host.browserGoBack` / `host.browserGoForward` / `host.browserReload` | 导航与历史；`reload` 可选 `hard` |
...
| `packages/client/ui-browser/` | **新包**（#96）：`conversation.details.browser` occupant；Workspace 级 Tab store + Client Zoom；首次进入 `about:blank` + 地址栏 focus；Tab 栏 + 导航栏 + screencast SSE + 指针/键盘转发；viewport debounce resize；未绑定空态；`packages/bundle/web-app` 注册 | 2026-08 |
...
| `issue/96-ui-browser-core` | [#96](https://github.com/NanGePlus/my-deepseek-harness/issues/96) `ui-browser`：about:blank + screencast + 未绑定空态 | 进行中 |
