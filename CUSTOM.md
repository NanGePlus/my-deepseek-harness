...
| `packages/client/ui-browser/` | **新包**（#96–#97）：`conversation.details.browser` occupant；Workspace 级 Tab store + Client Zoom；首次进入 `about:blank` + 地址栏 focus；Tab 栏 + 右键批量关闭 + 导航栏（← → ↻、地址栏、外部打开）+ screencast SSE + 指针/键盘转发；Host 历史态驱动 ← → disabled；viewport debounce resize；未绑定空态；`packages/bundle/web-app` 注册 | 2026-08 |
...
| `issue/97-ui-browser-tabs-nav` | [#97](https://github.com/NanGePlus/my-deepseek-harness/issues/97) `ui-browser`：多 Tab + 导航顶栏 | 进行中 |
