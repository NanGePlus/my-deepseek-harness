...
| 日期 | 操作 | 备注 |
|------|------|------|
| 2026-09-01 | 分支 `test/desktop-v5-validation`：工具箱浏览器报 `desktop browser CDP: no default context` | Playwright `browser.close()` 后 `contexts()` 变空但 Manager 仍缓存旧 handle。`ensureCdpBrowser` 检测 stale 并重连；`remote-allow-origins=*` |
