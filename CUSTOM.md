...
| `host.browserList` / `host.browserCreateTab` / `host.browserCloseTab` / `host.browserSelectTab` | Workspace 级 Playwright Tab 池列举 / 新建 / 关闭 / 选中 |
...
| `host.browserClick` / `host.browserType` / `host.browserScroll` / `host.browserSelectOption` | 页面交互 |
| `host.browserResizeViewport` / `host.browserSendPointer` / `host.browserSendKeyboard` | viewport 与指针/键盘转发 |
| `host.browserWatchScreencast` | SSE JPEG 帧（`page.screenshot` 轮询）；Client 断开不销毁 BrowserContext |
...
| `packages/client/connection/` | **2026-08-26** `host.watchPath` 浏览器下行改 WebSocket，避免 HTTP/1.1 六连接占满后文件树 listing 排队超时；**2026-08-30** connection schema 与 fetch client 扩展 `host.browser*` / `browserWatchScreencast` SSE | 2026-08 |
...
| `packages/client/ui-browser/` | **新包**（#96–#98）：`conversation.details.browser` occupant；Workspace 级 Tab store + Client Zoom；首次进入 `about:blank` + 地址栏 focus；Tab 栏 + 右键批量关闭 + 导航栏（← → ↻、地址栏、外部打开、… 溢出菜单）+ screencast SSE + 指针/键盘转发；不可用卡片 + 导航错误 inline/画布空态 + 外部站点 info；Host 历史态驱动 ← → disabled；viewport debounce resize；未绑定空态；`packages/bundle/web-app` 注册 | 2026-08 |
...
| `packages/client/runtime/` | `WorkspaceRuntime` 转发新 Host RPC；**2026-08-25** 转发 Git 面板只读 RPC；**2026-08-25** 转发 Git 面板写 RPC；**2026-08-27** `gitLog` 转发 `limit`/`skip` 与 `hasMore`；**2026-08-27** `gitCommitDiff`；**2026-08-27** `gitAddRemote`；**2026-08-27** `gitRemoveRemote`；**2026-08-28** `movePath`；**2026-08-29** 转发 `host.terminal*`（profiles/spawn/write/resize/list/stream）；**2026-08-30** 转发 `host.browser*` 与 `browserWatchScreencast` | 2026-08 |
