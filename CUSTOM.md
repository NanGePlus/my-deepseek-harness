...
| 日期 | 操作 | 备注 |
|------|------|------|
| 2026-09-02 | 分支 `test/desktop-v5-validation`：桌面端工作区列表为空 | 上一轮改 IPC bridge 时删掉 `api` 变量但 stream 打开仍引用它 → `ReferenceError: api is not defined`，mux/host 下行失败、工作区无法加载。从 `ctx.apiProxy` 恢复 stream 侧引用 |
