...
- **Git 面板 Graph**：待提交下方可折叠提交历史（主干 + 合入弧、分页 50）；每一行说明紧挨该行最右侧的点或线；引用胶囊 14px、长名省略；悬停固定定位详情卡（完整引用、作者、相对/绝对时间、说明、正文、短 hash）；Changes 打开时钉在操作列底部
- **Git 面板 Changes / Graph**：操作列两个同级可折叠组；**CHANGES** / **GRAPH** 段头 13px 加粗全大写；Changes 打开时 Graph 钉在底部（上限约半高）；Changes body（分支、提交区、两段变更列表）与 Graph 列表相对文件夹标题缩进 14px
...
| `packages/client/runtime/` | `WorkspaceRuntime` 转发新 Host RPC；**2026-08-25** 转发 Git 面板只读 RPC；**2026-08-25** 转发 Git 面板写 RPC；**2026-08-27** `gitLog` 转发 `limit`/`skip` 与 `hasMore` | 2026-08 |
...
| 2026-08-27 | Git Graph 引用胶囊 | 胶囊收窄并省略长分支名；悬停固定定位详情卡（完整引用、作者、时间、说明、正文、短 hash）；`gitLog` 增加 `%aI`/`%b` |
