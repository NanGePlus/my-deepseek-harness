...
| `packages/host/apiproxy/src/browser-registry.ts` | **#118**：`browserDelivery` web/desktop 分叉；desktop 走 `connectOverCDP`，跳过 `bringToFront`；**2026-08-31** 桌面 `revealTab` 调用 `surface.selectTab`；**2026-08-31** 桌面 `closeTab` 先 `surface.closeTab`；**2026-08-31** 桌面 `createTab` 在 `ensureTab` 后不再二次 Playwright `goto` | 2026-08-31 |
...
| 2026-08-31 | 分支 `test/desktop-v5-validation`：工具箱浏览器「更多操作」下拉被网页挡住 | Electron `BrowserView` 叠在 Renderer 之上，z-index / 向上展开都无法盖住 occupant 内像素。菜单打开时按菜单底边 inset occupant bounds，网页在菜单下方保持附着；关闭后恢复满格 |
