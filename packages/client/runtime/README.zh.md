...
`WorkspaceRuntime.listWorkspaceEntries`、`gitStatus`、`gitWorkingTree`、`gitInit`、`gitDiffPreview`、`readFile`、`writeFile`、`deletePath`、`renamePath` 与 `createWorkspaceDirectory` 转发同名 Host 一元 RPC。Host 业务失败抛出 `DirectoryBrowseError`。根目录不是 Git 仓库时，`gitStatus` 返回空列表。`gitWorkingTree` 对本机无 git 与不是仓库返回 `availability` 判别值，不抛错。
