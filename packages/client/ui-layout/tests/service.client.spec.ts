...
describe('LayoutController', () => {
  ...
  it('mirrors rendered details width into the detailsOpen observable', () => {
    const service = new LayoutController()
    const seen: boolean[] = []
    const unsub = service.detailsOpen.subscribe(() => {
      seen.push(service.detailsOpen.getSnapshot())
    })
    service.setDetailsOpen(true)
    service.setDetailsOpen(true)
    service.setDetailsOpen(false)
    unsub()
