...
describe('ui-layout client apply', () => {
  ...
  it('injects no business face and attaches the layout actions', async () => {
    ...
    await fiber.await()
    const actions = {
      setSidebar: vi.fn(), setDetails: vi.fn(), toggleSidebar: vi.fn(), openDetails: vi.fn(), closeDetails: vi.fn(), toggleDetails: vi.fn(),
    }
    const injected = (slots.entries('root')[0]!.inject as (actions: never) => object)(actions as never)
    expect(injected).toEqual({ syncDetailsOpen: expect.any(Function) })
