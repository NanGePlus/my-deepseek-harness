...
import { TreeSplitHandle } from '../src/client/TreeSplitHandle.tsx'
...
describe('TreeSplitHandle', () => {
  ...
  it('reports horizontal drag delta on pointer up', () => {
    ...
    const onEnd = vi.fn()
    const view = render(
      <TreeSplitHandle ariaLabel="Resize file tree" onStart={onStart} onDrag={onDrag} onEnd={onEnd} />,
    )
    const handle = view.getByRole('separator', { name: 'Resize file tree' })
    drag(handle, 400, 460)
