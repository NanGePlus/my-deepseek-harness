...
左栏展示当前分支（或 Git 对游离 HEAD 的说明）、带 **提交** 的提交说明输入，以及 **已更改，暂未选入提交** / **待提交** 列表。每段按内容高度排列，列表区整列滚动，因此未暂存行多时「待提交」被顶到后面。未暂存行提供整文件暂存与丢弃（丢弃须先确认）；已暂存行只提供取消暂存。提交要求非空说明与非空暂存列表；下拉可选 **提交并推送**（Host 在提交后执行 `git push`，无 upstream 时自动 `-u origin HEAD`）。草稿按 Session 存在槽位 store，切走 Git Tab 或切换 Session 都还在；提交成功后清空该 Session 草稿并显示 inline 成功提示。单击行在右栏加载只认磁盘的差异预览，不打开编辑器标签页。已跟踪文本可按差异块暂存、取消暂存或丢弃（丢弃须确认）；未跟踪文本、二进制与删除预览仅整文件操作。dirty 编辑器标签页会拦住丢弃、暂存（整文件、按块或 **全部暂存**）以及包含该路径的提交；取消暂存不受限。守卫对话框提示先显式保存、丢弃该编辑缓冲或关闭该标签页，不会自动保存。
