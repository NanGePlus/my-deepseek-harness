...
- **提交差异只对第一父提交且有上限** — Graph 选中后 `host.gitCommitDiff` 相对第一父提交（根提交相对空树），最多 80 个文件。文件头默认折叠，展开后才挂载该文件差异预览。
