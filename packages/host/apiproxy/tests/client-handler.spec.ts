...
function scriptedApi(overrides: {
  ...
  return {
    ...
    host: {
      ...
      gitWorkingTree: r => ok(r, { availability: 'not-a-repository' as const }),
      gitInit: r => ok(r, { repoRoot: '/t' }),
      gitDiffPreview: r => ok(r, { kind: 'binary' as const }),
      gitStage: r => ok(r, { availability: 'not-a-repository' as const }),
      ...
      gitAddRemote: r => ok(r, { availability: 'not-a-repository' as const }),
      gitRemoveRemote: r => ok(r, { availability: 'not-a-repository' as const }),
      gitLog: r => ok(r, { availability: 'not-a-repository' as const }),
      gitCommitDiff: r => ok(r, { availability: 'not-a-repository' as const }),
      ...
      browserNavigate: r => ok(r, { url: 'about:blank', title: '' }),
      browserGoBack: r => ok(r, { url: 'about:blank', title: '' }),
      browserGoForward: r => ok(r, { url: 'about:blank', title: '' }),
      browserReload: r => ok(r, { url: 'about:blank', title: '' }),
      browserSnapshot: r => ok(r, { tree: '' }),
      browserClick: r => ok(r, { clicked: true as const }),
      browserType: r => ok(r, { typed: true as const }),
      browserScroll: r => ok(r, { scrolled: true as const }),
      browserSelectOption: r => ok(r, { selected: true as const }),
      browserResizeViewport: r => ok(r, { resized: true as const }),
      browserSendPointer: r => ok(r, { sent: true as const }),
      browserSendKeyboard: r => ok(r, { sent: true as const }),
