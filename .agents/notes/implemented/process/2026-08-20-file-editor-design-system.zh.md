...
[`docs/design/DESIGN.md`](../../../../docs/design/DESIGN.md) 是文件编辑器品牌板。它把 Harness `--dsw-alias-*` / `--ds-font-family-*` 映射到通用原语（含 details 分段 Tab 与文件 Tab 栏），并在 §4 命名编辑器叠色 Token（`editor-hover-tint`、`editor-selected-tint`、`editor-danger-hover-tint`、`editor-tab-active-line`、`editor-dirty-dot`）。运行时 HEX 与 alias 解析仍在 [`ui-theme`](../../../../packages/client/ui-theme/README.md) 样式表；现行编码规则仍在 [web-styling.md](../../../../docs/web-styling.md) 与 [样式系统 Agent Note](2026-07-19-web-styling-system.md)。Git 面板消费同一品牌板，不另造色板或 §5 原语；见 [Git 面板设计系统 Agent Note](2026-08-25-git-panel-design-system.md)。
