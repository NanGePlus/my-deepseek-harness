...
Git occupant 是子槽 `conversation.details.git`（`kind: 'single'`，`scope: 'root'`）。`ui-git` 注入此处。三个 tabpanel 保持挂载；未选中的面板为 `display: none`（`aria-hidden`）。切走 Git 不取消暂存、不清空提交说明草稿——那属于 Git occupant，隐藏面板才能保留它们。

在 `ui-git` 注册之前，空的 `conversation.details.git` 席位就是正确的组装态。右栏拖宽与 concession 仍在 `ui-layout`。
...
- 浏览器 e2e `details-segmented-tab` 包含 Git Tab 文案。Git 面板空态不属于本切片。
