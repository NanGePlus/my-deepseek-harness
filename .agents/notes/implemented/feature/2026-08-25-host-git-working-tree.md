...
`host.gitWorkingTree({ workspaceId })` walks upward from the bound Workspace with `git rev-parse --show-toplevel`, returns the current branch or Git’s detached-HEAD description, and lists unstaged and staged disk changes. Each row carries a POSIX path relative to the repository root, a Host-absolute `absolutePath` that may lie outside the bound Workspace, and `kind` (`modified` / `untracked` / `deleted`) so the Git panel can pick discard-confirm copy without a preview round-trip. Ignored paths are omitted (default porcelain). Git missing and not-a-repository are success `availability` values, not RPC errors, so the panel can render the matching empty state. Unexpected git failures remain `internal`.
